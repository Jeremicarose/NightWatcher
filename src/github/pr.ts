...
`;
    ...
    const { data: pr } = await octokit.pulls.create({
      owner,
      repo: repoName,
      title: `🔧 Nightwatch Auto-Fix: ${analysis.error_type} in ${fix.file_path}`,
      body: prBody,
      head: branchName,
      base: branch,
    });
...
`;
  ...
  if (fixAttempts.length > 0) {
    ...
    if (successfulAttempt) {
      body += `✅ All tests passing (fixed on attempt ${successfulAttempt.attempt_number})\n`;
    }
  }

  body += `
---
*This PR was generated autonomously by [Nightwatch](https://github.com/Jeremicarose/NightWatcher) 🌙*
`;
...
export async function createEscalationIssue(context: IssueContext): Promise<IssueResult> {
  ...
  try {
    ...
    const { data: issue } = await octokit.issues.create({
      owner,
      repo: repoName,
      title: `🚨 Nightwatch needs help: ${analysis.error_type} in ${analysis.file_path}`,
      body: issueBody,
...
- **Result:** ${attempt.test_result === 'pass' ? '✅ Passed' : '❌ Failed'}
`;
...
---
*This issue was created by [Nightwatch](https://github.com/Jeremicarose/NightWatcher) 🌙*
`;
